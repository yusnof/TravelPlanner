...
shortestPath graph from to =   Just (findPath set from to graph [], fromJust (M.lookup to set))
    where
     --set:: Map a b
...
findPath set start stop graph list
    | start == stop = list
    | otherwise = findPath set newStart stop graph newList
        where
            adjacent = adj start graph
            newStart =  dst (fromJust (smallestOfAdjacent adjacent set))
            newList = list ++ [newStart]

smallestOfAdjacent :: (Ord b, Ord a, Show a, Show b) => [Edge a b] -> Map a b -> Maybe (Edge a b) 
smallestOfAdjacent [] set = Nothing
smallestOfAdjacent [x] set =  Just x
smallestOfAdjacent (x:xs) set = case M.lookup (src x) set of
    Nothing -> Just x
    Just y -> Just (smallest x y)
  where 
    smallest edge value = if label edge < value 
      then edge 
      else smallest (head xs) value

{- smallestOfAdjacent :: (Ord b, Ord a, Show a, Show b) => [Edge a b] -> Map a b -> Maybe (Edge a b)
smallestOfAdjacent edges valueMap =
    case edges of
        [] -> Nothing
        (Edge a b c : rest) ->
            case M.lookup a valueMap of
                Just value | c < value -> Just (Edge a b c)
                _ -> smallestOfAdjacent rest valueMap -}
...
recursive queue map graph to 
  ...
  where
    currentKey = PQ.key (fromJust (PQ.findMin queue))
    currentDist = PQ.prio (fromJust (PQ.findMin queue))
